...
def build_result_embed(result: dict, anilist_info: dict | None, page: int, total: int) -> hikari.Embed:
    ...
    lines.append(fmt_ts(result.get("from", 0)))
    ...
    embed.set_footer(text=f"Result {page}/{total} | Powered by trace.moe")
    return embed

async def fetch_whatanime_results(session: aiohttp.ClientSession, image_url: str) -> tuple[list[dict], dict]:
    ...
    if not results:
        raise RuntimeError("Không có kết quả.")
...
async def build_whatanime_embed(session: aiohttp.ClientSession, image_url: str) -> tuple[hikari.Embed | None, str | None]:
    try:
        ...
        anilist_info = anilist_data.get(results[0].get("anilist"))
        embed = build_result_embed(results[0], anilist_info, 1, len(results))
        return embed, None
